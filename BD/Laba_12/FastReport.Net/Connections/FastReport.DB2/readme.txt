DB2 data connection for FastReport.Net
----------------------------------------
...
- you need the DB2 .Net Data Provider to be installed. Obtain it
at http://www-01.ibm.com/software/data/db2/9/download.html;
- open the .sln file;
...
- check that "IBM.Data.DB2" reference is correct. 
If Visual Studio complains about it, fix this reference as well. By default,
this dll is located in the GAC;
- compile the project. You will get the "FastReport.DB2.dll" file.
...
- on the "Plugins" tab, add the FastReport.DB2.dll;
...
- open the config file in any text editor and modify it in the following way:
  ...
  <Config>
    ...
    <Plugins>
      <Plugin Name="c:\.....\FastReport.DB2.dll"/>
    </Plugins>
  </Config>

3) register the dll programmatically
- add the "FastReport.DB2.dll" reference to your project;
- execute the following code once at the application start:
FastReport.Utils.RegisteredObjects.AddConnection(typeof(DB2DataConnection));


How to use it:
--------------

- now you should be able to create a new DB2 data source in the 
